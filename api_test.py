...
class CogsApiMyTestCase(CogsApiObjectHelpers, CogsApiTestCase):
    ...
    def test_my_submissions_runs(self):
        ...
        url_lst_filter = '/my/submissions/{}/runs/'.format(sub_uuid)
        url_obj = '/runs/'
        obj_key = 'runs'
...
class CogsApiUserTestCase(CogsApiObjectHelpers, CogsApiTestCase):
    ...
    def test_user_assignment_submissions(self):
        ...
        url_lst_filter_user = '/users/{:s}/assignments/{:s}/submissions/'.format(self.nonadmin_uuid,
                                                                                 asn_uuid)
        url_lst_filter_admin = '/users/{:s}/assignments/{:s}/submissions/'.format(self.admin_uuid,
                                                                                  asn_uuid)
        url_obj = '/submissions/'
        obj_key = 'submissions'
    ...
    def test_user_runs(self):
        ...
        # Set URLs
        url_create = '/submissions/{}/runs/'.format(sub_uuid)
        url_lst_filter_user = '/users/{}/runs/'.format(self.nonadmin_uuid)
        url_lst_filter_admin = '/users/{}/runs/'.format(self.admin_uuid)
        url_obj = '/runs/'
        obj_key = 'runs'
        data = copy.copy(cogs.test_common.RUN_TESTDICT)
        data['test'] = tst_uuid
    ...
    def test_user_uuid_get(self):
        ep = '{}useruuid/{}/'.format(self.user_url, self.admin['username'])
        res = self.open_user('GET', ep, user=self.admin)
        self.assertEqual(res.status_code, 200)
        res_obj = json.loads(res.data)
        self.assertTrue(res_obj)
